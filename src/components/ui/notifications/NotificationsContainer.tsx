...
import React, { useCallback, useImperativeHandle, forwardRef, useMemo, useRef, useState, useEffect } from 'react';
...
interface IProps {
  ...
  verticalPosition: ENotificationsContainerVerticalPosition;
  horizontalPosition: ENotificationsContainerHorizontalPosition;
  width?: string;
...
export enum ENotificationsContainerVerticalPosition {
  Top,
  Bottom,
}

export enum ENotificationsContainerHorizontalPosition {
  Left,
  Center,
  Right,
}

export interface INotificationsContainerHandlers {
  clearAll: () => void;
  addNotification: (
    type: ENotificationType,
    body: string | React.ReactNode,
...
export const NotificationsContainer: React.FC<IProps> = props => {
  const {
  ...
  const alignSelf = useMemo((): string => {
    if (horizontalPosition === ENotificationsContainerHorizontalPosition.Left) {
      return 'flex-start';
    } else if (horizontalPosition === ENotificationsContainerHorizontalPosition.Right) {
      return 'flex-end';
    } else {
      return 'center';
    }
  ...
  const rootTop = useMemo((): string => {
    if (verticalPosition === ENotificationsContainerVerticalPosition.Top) {
  ...
  const rootBottom = useMemo((): string => {
    if (verticalPosition === ENotificationsContainerVerticalPosition.Bottom) {
      return verticalOffset;
    } else {
      return 'auto';
    }
